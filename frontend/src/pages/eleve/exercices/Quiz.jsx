  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // 🔥 Correction : On extrait uniquement la liste des exercices non faits "aFaire"
    ...
    return (
      <div className={`ex-root ${dark ? "dark" : "light"}`}>
        <div className="ex-loading-full">
          ...
          <button className="ex-back-btn" style={{ marginTop: "20px", position: "static" }} onClick={() => navigate("/eleve/exercices")}>
            {t.retour}
          </button>
        </div>
      </div>
